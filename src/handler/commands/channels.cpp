...
void Handler::_cmd_invite(Message &msg, User &user)
{
	...
	if (_server.map_channels[channel_name]->getFlags() & INVITE_ONLY and !_server.map_channels[channel_name]->_is_user_operator(user.getNick()))
		return _error_msg(user, 482, channel_name);
	// else DONE! подключить к каналу)
	_cmd_responses(nick + " " + channel_name, user, 341);
	_server.map_channels[channel_name]->_join_user(*_server.mapnick_users[nick], "", true);
...
void Handler::_cmd_topic(Message &msg, User &user)
{
	...
	else
	{
		...
		if (msg.get_params().size() == 1)
		{
			channel->getTopic().empty() ? _cmd_responses(channel->getName(), user, 331) : _cmd_responses(channel->getName() + " :" + channel->getTopic(), user, 332);
		}
